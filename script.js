...
const cellSize = 20;
const maxSize = 30;
let mousePos = vec2.fromValues(innerWidth * 0.15, innerHeight * 0.1);
let numThingsX;
let numThingsY;
let things;
...
function drawThing(thing) {
  ...
  ctx.beginPath();
  ctx.arc(pos[0], pos[1], radius, 0, Math.PI * 55);
  ctx.fill();
}
...
function loop() {
  ...
  things.forEach(thing => {
    ...
    thing.radius = clamp(dist * dist * 0.0001 - .5, 0, maxSize);
    drawThing(thing);
  });
...
function makeThings() {
  ...
  for (let i = 0; i < numThingsY; i += 1) {
    for (let j = 0; j < numThingsX; j += 1) {
      const thing = makeThing(j * cellSize + cellSize * 2, i * cellSize + cellSize * 2);
      things.push(thing);
    }
  }
}
